...
public:
  ...
  Area(int H, int W, char out_val) : H(H), W(W), data(vector(H, string(W, '.'))), allow_outside(true), out_val(out_val) {}
  ...
  char at(int x, int y) {
    if(is_contained(H, W, x, y)) return data[x][y];
    else if(allow_outside) return out_val;
    else {
  ...
  void set(int x, int y, char val) {
    if(is_contained(H, W, x, y)) data[x][y] = val;
    else if(allow_outside) return;
    else {
