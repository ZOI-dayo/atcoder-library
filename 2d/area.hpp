...
public:
  ...
  Area(int H, int W, char out_val)
      : H(H), W(W), data(std::vector(H, std::string(W, '.'))), allow_outside(true),
        out_val(out_val) {}
  std::string &operator[](int i) { return data[i]; }
  // 時計回りで90度回転
  Area rotated90() const {
    Area ret(W, H);
    for(int i=0; i<H; ++i) for(int j=0; j<W; ++j) ret[j][H - i - 1] = data[i][j];
    return ret;
  }
  void rotate90() {
    assert(H == W);
  ...
  bool contains(int x, int y) const { return 0 <= x && x < H && 0 <= y && y < W; }
...
inline std::ostream &operator<<(std::ostream &os, Area &a) {
  for(int i=0; i<a.H; ++i) os << a[i] << std::endl;
  return os;
}
