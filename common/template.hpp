...
#define printd(x) cerr << _TERM_BOLD << _TERM_BACK_RED << "[debug] " << #x << " = " << x << _TERM_BACK_RESET << _TERM_DECO_RESET << endl;


// --- Utils ---

// 合計値を求める
ll sum(const vec<ll> &v) {
...
// 半開区間[x, z)にyが含まれるか?
bool is_contained(int x, int y, int z) {
  return x <= y && y < z;
}
