...
public:
  ...
  inline LazySegmentTree(const int32_t n) : _monoid(M()), _monofunc(F()), e(_monoid.e()), id(_monofunc.id()), {
    _n = 1;
    _height = 1;
    while (_n < n)
      _n <<= 1, _height++;
    ...
    */
    ...
    while ((a >>= 1), (b >>= 1), a) {
      if (_lazy[a] == id) {
        ...
        _data[a] = op(_monofunc.apply(_lazy[(a << 1) | 0], _data[(a << 1) | 0]),
      ...
      if (_lazy[b] == id) {
        ...
        _data[b] = op(_monofunc.apply(_lazy[(b << 1) | 0], _data[(b << 1) | 0]),
