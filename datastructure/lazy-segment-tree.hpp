...
public:
  ...
  inline void set(int l, int r, F f) {
    ...
    */
    ...
    // 上から下へとlazyを伝搬させる
    for (int i = _height; 0 < i; --i) eval(a >> i), eval(b >> i);
    // bを [a, b]から[a, b)にする
    for(b++; a < b; a >>=1, b >>=1) {
      if(a&1) _lazy[a] = merge(f, _lazy[a]), eval(a), a++;
      if(b&1) --b, _lazy[b] = merge(f, _lazy[b]), eval(b);
    }
...
=======
  ...
  inline T query(int l, int r) {
    ...
    */
    ...
    // bを [a, b]から[a, b)にする
    for(b++; a < b; a >>=1, b >>=1) {
      if(a&1) vl = op(vl,_data[a]=update(_lazy[a], _data[a])),a++;
      if(b&1) b--,vr = op(_data[b]=update(_lazy[b], _data[b]),vr);
    }
    // line_debug();
    return op(vl,vr);
  }
