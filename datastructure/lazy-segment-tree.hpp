...
#define P(i) (i >> 1)
      ...
      /**
       * @brief k番目のノードの遅延情報を解消し、子ノードに伝搬する O(log N) *
       * merge
       *
    ...
    public:
       ...
       * @param merge 操作のマージ
       */
      explicit inline LazySegmentTree(const uint32_t length, const T e,
                                      const F id,
                                      const std::function<T(T, T)> op,
                                      const std::function<T(F, T)> apply,
                                      const std::function<F(F, F)> merge) :
       ...
       * @param merge 操作のマージ
       */
      explicit inline LazySegmentTree(std::vector<T>& data, const T e,
                                      const F id,
                                      const std::function<T(T, T)> op,
                                      const std::function<T(F, T)> apply,
                                      const std::function<F(F, F)> merge) :
      ...
      inline void set(int32_t l, int32_t r, F f) {
        ...
        // 上から下へとlazyを伝搬させる
        for (int32_t i = _height; 0 < i; --i) eval(a >> i), eval(b >> i);
        // bを [a, b]から[a, b)にする
        for (b++; a < b; a >>= 1, b >>= 1) {
      ...
      inline T query(int32_t l, int32_t r) {
        ...
        // bを [a, b]から[a, b)にする
        for (b++; a < b; a >>= 1, b >>= 1) {
          if (a & 1) vl = op(vl, apply(_lazy[a], _data[a])), a++;
          if (b & 1) b--, vr = op(apply(_lazy[b], _data[b]), vr);
        }
        return op(vl, vr);
      }
