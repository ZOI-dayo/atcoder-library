...
namespace zoi {
  namespace datastructure {
    ...
    private:
      ...
      struct Node {
        ...
        Node(T val, T e, int l, int r) :
            val(val), e(e), p(nullptr), l_child(nullptr), r_child(nullptr),
            l(l), r(r) {}
    ...
    public:
       ...
       * @param op 演算
       */
      inline explicit DynamicSegmentTree(int length, T e,
                                         std::function<T(T, T)> op) :
      ...
      inline Node* get_node(int i) const {
        Node* node = root;
        while (node->l + 1 < node->r) node = node->child(node->mid() <= i);
        return node;
      }
    ...
    private:
      /**
       * @brief 特定のノードに対し、[l, r)の範囲に対するクエリを行う O(log n) *
       * op
       *
