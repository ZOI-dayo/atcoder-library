    ...
    namespace __imos2d {
      ...
      public:
        ...
        explicit imos2d(std::vector<std::vector<T>> src) :
            _H(src.size()), _W(src[0].size()),
            _data(_H + 1, vec<T>(_W + 1, 0)) {
          for (int x = 0; x < _H; ++x)
            for (int y = 0; y < _W; ++y) add({x, y}, src[x][y]);
        }
        ...
        inline void add(point_t a, point_t b, T x) {
          ...
          assert(is_contained(_H, _W, a) &&
                 is_contained(_H, _W, b.upper_left()));
          _data[a.x][a.y] += x;
          _data[a.x][b.y] -= x;
          _data[b.x][a.y] -= x;
          _data[b.x][b.y] += x;
        ...
        inline void build() {
          for (int x = 0; x < _H; ++x)
            for (int y = 0; y < _W; ++y) _data[x][y + 1] += _data[x][y];
          for (int y = 0; y < _W; ++y)
            for (int x = 0; x < _H; ++x) _data[x + 1][y] += _data[x][y];
          _is_build = true;
        }
