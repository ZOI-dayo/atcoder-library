...
namespace zoi {
  namespace convolution {
    ...
    namespace __convolution {
      ...
      // | views::transform([](int n) { return mint(ROOT).pow(1LL <<
      // (DIVIDE_LIMIT - n)); });
      ...
      // constinit const auto iws = views::iota(0, DIVIDE_LIMIT + 1) |
      // views::all | views::transform([](int n) { return ws[n].pow(MOD - 2);
      // });
